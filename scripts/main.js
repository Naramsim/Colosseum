...
App.controller('MainPokemon', function($scope, $http, $rootScope) {
	$scope.$on('init', function(){
		...
		$scope.name = up($rootScope.currentPokemon.name);                
		$scope.id = $rootScope.currentPokemon.id;                
		$scope.height = $rootScope.currentPokemon.height;                
		$scope.weight = $rootScope.currentPokemon.weight; 
		$scope.abilities = $rootScope.currentPokemon.abilities;               
		$scope.stats = $rootScope.currentPokemon.stats;               
		$scope.moves = $rootScope.currentPokemon.moves;               
		$scope.held_items = $rootScope.currentPokemon.held_items;
...
App.controller('PokemonFamily', function($scope, $rootScope, $http) {
	$scope.$on('init', function(){
		$scope.getPokemonName = function(id) {
		...
		$http.get($rootScope.pokemonSpecie.evolution_chain.url)
    ...
    function($http, $q) {
        return {
            getPokemon: function() {
                ...
                $http.get(`https://pokeapi.co/api/v2/pokemon/${pokemons.indexOf(currentPokemon) + 1}/`)
				   .success(function(res){
            ...
            getSpecie: function(element) {
                var deferred = $q.defer();
            	$http.get(`https://pokeapi.co/api/v2/pokemon-species/${pokemons.indexOf(currentPokemon) + 1}/`)
			    	.success(function(res){
			    		deferred.resolve(res);
				    })
				   .error(function(data) {
